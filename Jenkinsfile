...
node("master") {
   ...
   unstash 'msi'
   sh "ls -l"
   sh "curl -v -i -X POST -H \"Content-Type:application/json\" -H \"Authorization: token ${github_token}\" https://api.github.com/repos/bacgroup/man-ads-client/releases -d '{\"tag_name\":\"man_ovd_client_${BUILD_NUMBER}\",\"target_commitish\": \"${BRANCH_NAME}\",\"name\": \"MAN OVD Client Build ${BUILD_NUMBER}\",\"body\": \"MAN Consulting Software\",\"draft\": false,\"prerelease\": true}'"
   sh "for i in *.msi; do bash $HOME/github-release.sh github_api_token=${github_token} owner=bacgroup repo=man-ads-client tag=man_ovd_client_${BUILD_NUMBER} filename=./\$i; done"
}
